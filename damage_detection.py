...
def calculate_areas(points, im):
    ...
    for ii in range(1):
        ...
        cx, cy, r1 = get_circle(p1, p2, p3)
        cv2.circle(im1, (cx, cy), r1, 255, -1)
        # Circle on apple skin:
        cx, cy, r2 = get_circle(p1, p4, p3)
        cv2.circle(im2, (cx, cy), r2, 255, -1)
        ...
        ret.append([area, cnt, color, r1, r2])
        ...
        while True:
            ...
            if k == ord("c"):
                if len(points) == 4:
                    scale = get_scale(img)
                    print(scale)
                    # (area1, cnt1, color1, r1), (area2, cnt2, color2, r2) = calculate_areas(points, im_to_show)
                    print(calculate_areas(points, im_to_show))
                    area1, cnt1, color1, r1 = calculate_areas(points, im_to_show)[0]
                    res = [(area1*scale, cnt1, color1, r1)]  # , (area2*scale, cnt2, color2, r2)
...
def main_processing(img_path, dict_path):
    ...
    for label, variety in varieties_dict.items():
        ...
        if not os.path.exists(variety_im_path):
            break
    ...
    with open(csv_filename, "w", newline='') as csv_file:
        ...
        csv_writer.writerow(["label", "apple ID", "variety", "repetition", "area (mm2)", "r1", "r2", "g", "b"])
        for label, variety, im_id, img, points, res in data:
            for ii, (area, cnt, (b, g, r1, r2)) in enumerate(res):
                csv_writer.writerow([label, im_id, variety, ii+1, area, r1, r2, g, b])
